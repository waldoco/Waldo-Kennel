...
describe("BoundedComposer", () => {
	it("carries typed text back to the host via onChange", () => {
		...
		render(<BoundedComposer inputLabel="Message the mission" onChange={onChange} onSubmit={vi.fn()} submitLabel="Send" value="" />);
		fireEvent.change(screen.getByRole("textbox"), { target: { value: "hi" } });
		expect(onChange).toHaveBeenCalledWith("hi");
	});

	it("disables submit until there is non-whitespace content", () => {
		render(<BoundedComposer inputLabel="Message the mission" onChange={vi.fn()} onSubmit={vi.fn()} submitLabel="Send" value="   " />);
		expect(screen.getByTestId("bounded-composer-submit")).toBeDisabled();
	});

	it("submits on click when there is content", () => {
		const onSubmit = vi.fn();
		render(<BoundedComposer inputLabel="Message the mission" onChange={vi.fn()} onSubmit={onSubmit} submitLabel="Send" value="do the thing" />);
	...
	it("distinguishes disabled (host policy) from pending (activity), both by visible text", () => {
		const { rerender } = render(<BoundedComposer disabled inputLabel="Message the mission" onChange={vi.fn()} onSubmit={vi.fn()} submitLabel="Send" value="x" />);
		expect(screen.getByRole("textbox")).toBeDisabled();
		expect(screen.getByTestId("bounded-composer-submit")).toHaveTextContent("Send");

		rerender(<BoundedComposer inputLabel="Message the mission" onChange={vi.fn()} onSubmit={vi.fn()} pending submitLabel="Send" value="x" />);
		expect(screen.getByRole("textbox")).toBeDisabled();
	...
	it("renders host-owned scope and source disclosure, and neither by default", () => {
		const { rerender } = render(<BoundedComposer inputLabel="Message the mission" onChange={vi.fn()} onSubmit={vi.fn()} submitLabel="Send" value="" />);
		...
		rerender(
			<BoundedComposer
				inputLabel="Message the mission"
				onChange={vi.fn()}
				onSubmit={vi.fn()}
	...
	it("exposes the textarea to the AX tree under its accessible name", () => {
		render(<BoundedComposer inputLabel="Message the mission" onChange={vi.fn()} onSubmit={vi.fn()} placeholder="Type…" submitLabel="Send" value="" />);
