...
describe("parseRunFile", () => {
	...
	it("parses protected harness locators without any credential", () => {
		expect(parseRunFile(JSON.stringify({ pid: 4242, port: 3037, harnessPairingAddress: "/tmp/kennel-pair.sock", harnessCommandAddress: "/tmp/kennel-command.sock" }))).toEqual(
			expect.objectContaining({ harnessPairingAddress: "/tmp/kennel-pair.sock", harnessCommandAddress: "/tmp/kennel-command.sock" }),
		);
	});
