...
const LISTEN_LINE =
  'time=2026-06-10T09:15:04.221-07:00 level=INFO msg="daemon listening" addr=127.0.0.1:3001 pid=4242';
...
describe("parseDaemonListenPort", () => {
  ...
  it("reports a fallback port that differs from the configured one", () => {
    expect(parseDaemonListenPort(LISTEN_LINE.replace("3001", "3037"))).toBe(
      3037,
    );
  });
  ...
  it("handles IPv6 listen addresses", () => {
    expect(
      parseDaemonListenPort(
        'level=INFO msg="daemon listening" addr=[::1]:4317 pid=1',
      ),
    ).toBe(4317);
  });
  ...
  it("ignores other log lines, even ones carrying an addr attribute", () => {
    expect(
      parseDaemonListenPort('level=INFO msg="proxy dial" addr=127.0.0.1:9999'),
    ).toBeNull();
  ...
  it("returns null when the listen line has no usable addr", () => {
    expect(
      parseDaemonListenPort('level=INFO msg="daemon listening" pid=4242'),
    ).toBeNull();
    expect(
      parseDaemonListenPort('level=INFO msg="daemon listening" addr=garbage'),
    ).toBeNull();
    expect(
      parseDaemonListenPort(
        'level=INFO msg="daemon listening" addr=127.0.0.1:99999',
      ),
    ).toBeNull();
  });
});
...
describe("parseRunFile", () => {
    ...
    expect(
      parseRunFile(
        JSON.stringify({
          pid: 4242,
          port: 3037,
          supervisorAddress: "/tmp/kennel-supervise.sock",
        }),
      ),
    ).toEqual(
      expect.objectContaining({
  ...
  it("parses protected harness locators without any credential", () => {
    expect(
      parseRunFile(
        JSON.stringify({
          pid: 4242,
          port: 3037,
          harnessPairingAddress: "/tmp/kennel-pair.sock",
          harnessCommandAddress: "/tmp/kennel-command.sock",
        }),
      ),
    ).toEqual(
      expect.objectContaining({
  ...
  it("parses the app run that owns the daemon browser credential", () => {
    expect(
      parseRunFile(
        JSON.stringify({ pid: 4242, port: 3037, appRunId: "apprun-current" }),
      ),
    ).toEqual(expect.objectContaining({ appRunId: "apprun-current" }));
  });
  ...
  it("returns null for a missing or invalid port", () => {
    expect(
      parseRunFile(
        JSON.stringify({ pid: 1, startedAt: "2026-06-10T16:15:04Z" }),
      ),
    ).toBeNull();
  ...
  it("treats a missing or unparseable startedAt as epoch zero", () => {
    expect(
      parseRunFile(JSON.stringify({ pid: 1, port: 3001 }))?.startedAtMs,
    ).toBe(0);
    expect(
      parseRunFile(
        JSON.stringify({ pid: 1, port: 3001, startedAt: "not-a-date" }),
      )?.startedAtMs,
    ).toBe(0);
  });
});
...
describe("defaultRunFilePath", () => {
  it("matches Go's canonical Kennel home default on macOS", () => {
    expect(defaultRunFilePath("darwin", {}, "/Users/me")).toBe(
      "/Users/me/.kennel/running.json",
    );
  });

  it("ignores XDG_CONFIG_HOME on linux", () => {
    expect(
      defaultRunFilePath("linux", { XDG_CONFIG_HOME: "/xdg" }, "/home/me"),
    ).toBe("/home/me/.kennel/running.json");
    expect(defaultRunFilePath("linux", {}, "/home/me")).toBe(
      "/home/me/.kennel/running.json",
    );
  });
  ...
  it("ignores APPDATA on windows", () => {
    ...
    ).toBe("C:\\Users\\me/.kennel/running.json");
    expect(defaultRunFilePath("win32", {}, "C:\\Users\\me")).toBe(
