...
function ResultSummary({ proof }: { proof: OutcomeProofRecord }) {
	...
	return (
		<section className="grid gap-3 rounded-md border border-border bg-card/50 p-4 sm:grid-cols-3" data-testid="outcome-result-summary">
			...
			{gaps.length > 0 && <div className="sm:col-span-3"><p className="text-xs uppercase tracking-wide text-warning">{t("outcome.proof.limitations")}</p><ul className="mt-1 list-disc pl-4 text-xs text-muted-foreground">{gaps.map((gap, index) => <li key={`${index}-${gap}`}>{gap}</li>)}</ul></div>}
