...
After F0 + F1 (identical build command): **5,134,083 B raw / 1,408,766 B gzip** — **+630 B / +9 B gzip**, same 51 files by name. Re-running the identical post-change tree a second time reproduces the exact same total deterministically, and a per-chunk diff attributes the entire delta to one already-existing chunk (`SessionView`, −5 B) plus filename-hash churn — consistent with the product-ui module graph growing by eight always-dead-code exports that get tree-shaken away, with a negligible knock-on effect on an unrelated chunk's minified variable names. No new chunk is emitted; `@xyflow/react` does not reach the production bundle.
