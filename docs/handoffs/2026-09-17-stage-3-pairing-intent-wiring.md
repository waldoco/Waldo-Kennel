...
2. Electron main (or another existing authenticated owner client) sends that closed tuple to the daemon's loopback-only `/internal/owner-commands/harness-pairing-intents` route with the inherited `KennelOwner` token.
3. The route authenticates through the existing `ownercommand.Authority`, rejects unknown fields/classes, fixes challenge and connection expiry on the daemon side, and calls `Coordinator.Issue`.
...
- Exact request body is bounded and unknown fields are rejected.
