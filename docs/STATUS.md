...
| 4 Harness installation and connection | Partial | Trusted install/drift, pairing, connection kernel and reconnect exist. Stage 4C focused falsifiers pin exact drift repair mapping, generic wire rejection before claim creation, and stale run-file service/PID ownership. Public approval/revocation UI API, packaged journey, listener promotion and Mac live proof remain. |
