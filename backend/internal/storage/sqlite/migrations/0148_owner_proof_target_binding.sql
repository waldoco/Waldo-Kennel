...
CREATE TRIGGER chat_command_targets_identity_immutable BEFORE UPDATE ON chat_command_targets
WHEN NEW.session_id<>OLD.session_id
BEGIN SELECT RAISE(ABORT,'chat command target identity is immutable'); END;
...
CREATE TABLE command_authority_claims(
 ...
 connection_binding_digest TEXT NOT NULL CHECK(length(connection_binding_digest)=64), connection_expires_at TIMESTAMP NOT NULL, connection_revoked_at TIMESTAMP, transport_class TEXT NOT NULL,
...
-- +goose Down
DROP TRIGGER IF EXISTS chat_command_targets_identity_immutable;
DROP TABLE IF EXISTS chat_command_targets;
