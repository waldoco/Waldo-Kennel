-- A replacement controller may adopt a governed control command only before
-- dispatching. Dispatching is the durable pre-effect boundary and remains
...
WHEN NEW.id <> OLD.id OR NEW.session_id <> OLD.session_id
 ...
 OR NEW.expected_revision <> OLD.expected_revision OR NEW.capability_fingerprint <> OLD.capability_fingerprint
 OR NEW.provider_conversation_id <> OLD.provider_conversation_id OR NEW.client_message_id <> OLD.client_message_id
 OR NEW.provider_turn_id <> OLD.provider_turn_id OR NEW.request_instance_id <> OLD.request_instance_id
...
WHEN NEW.id <> OLD.id OR NEW.session_id <> OLD.session_id
 OR NEW.idempotency_key <> OLD.idempotency_key OR NEW.request_fingerprint <> OLD.request_fingerprint
 OR NEW.command_class <> OLD.command_class OR NEW.controller_generation <> OLD.controller_generation
 OR NEW.expected_revision <> OLD.expected_revision OR NEW.capability_fingerprint <> OLD.capability_fingerprint
 OR NEW.provider_conversation_id <> OLD.provider_conversation_id OR NEW.client_message_id <> OLD.client_message_id
 OR NEW.provider_turn_id <> OLD.provider_turn_id OR NEW.request_instance_id <> OLD.request_instance_id
BEGIN SELECT RAISE(ABORT, 'governed control command identity is immutable'); END;
