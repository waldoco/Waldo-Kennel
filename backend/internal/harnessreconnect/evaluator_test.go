...
type missions struct{ ok bool }

func (m missions) HarnessMissionExists(context.Context, string) (bool, error) { return m.ok, nil }
...
func TestEvaluatePrecedence(t *testing.T) {
	cases := []struct {
		...
		reason string
	}{{"daemon", func(_ *Request, e *Evaluator) { e.Daemon = nil }, string(domain.ReconnectReasonDaemonUnreachable)}, {"mission", func(_ *Request, e *Evaluator) { e.Missions = missions{} }, string(domain.ReconnectReasonMissionNotFound)}, {"delivery", func(r *Request, e *Evaluator) {
